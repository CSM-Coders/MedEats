...
def generate_restaurant_menu_pdf(restaurant_name, category_name, seed=None):
    ...
    dishes = DISHES_BY_CATEGORY.get(
        category_name, DISHES_BY_CATEGORY["Vegetarian & Healthy"]
    )
        ...
        TableStyle(
            [
                (
                    ...
                    (0, 0),
                    (-1, 0),
                    colors.HexColor(get_category_color(category_name)),
                ),
    ...
    for idx, restaurant in enumerate(restaurants, 1):
        try:
            ...
            restaurant.image.save(
                img_filename, ContentFile(img_buffer.read()), save=False
            )
            ...
            restaurant.menu_pdf.save(
                pdf_filename, ContentFile(pdf_buffer.read()), save=False
            )
            ...
            if branches.exists():
                ...
                print(
                    f"  ✓ {branches.count()} sedes actualizadas (heredan foto y menú)"
                )
